...
unsigned long __init arch_syscall_addr(int nr)
{
	...
	if (nr >= __NR_N32_Linux && nr < __NR_N32_Linux + __NR_compatn32_syscalls)
		return (unsigned long)sysn32_call_table[nr - __NR_N32_Linux];
#endif
	if (nr >= __NR_64_Linux  && nr < __NR_64_Linux + __NR_syscalls)
		return (unsigned long)sys_call_table[nr - __NR_64_Linux];
#ifdef CONFIG_MIPS32_O32
	if (nr >= __NR_O32_Linux && nr < __NR_O32_Linux + __NR_compato32_syscalls)
